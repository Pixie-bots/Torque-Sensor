...
uint32_t adc_val_1 = 0; // Initialize to 0 (no valid value until updated)
                          ...
                          ke_task_id_t const src_id)
{
    switch(msgid)
    {
        ...
        case CUSTS1_VALUE_REQ_IND:
        {
            ...
            // Debug: Log attribute index
            // printf("CUSTS1_VALUE_REQ_IND: att_idx=0x%04x\n", msg_param->att_idx);
            ...
            switch (msg_param->att_idx)
            {
                ...
                case SVC1_IDX_ADC_VAL_1_VAL:
                {
                    ...
                    // Use the global ADC value
                    uint32_t big_endian_val = __builtin_bswap32(adc_val_1);
                    memcpy(rsp->value, &big_endian_val, DEF_SVC1_ADC_VAL_1_CHAR_LEN);

                    // Debug: Log read value
                    // printf("Read ADC Val 1: value=0x%08x\n", big_endian_val);
